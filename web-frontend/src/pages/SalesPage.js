...
const formatDateTime = (value) => {
  ...
  if (typeof value === "string" && value.includes(" ")) {
    // "2025-11-13 13:46:12" → "2025-11-13 13:46"
...
export default function SalesPage() {
  ...
  const [items, setItems] = useState([]); // 개별 상품 기준 행

  const categoryColorMap = useMemo(() => {
    const names = [...new Set((overviewCategory || []).map((d) => d.category))].sort();
    const map = {};
    names.forEach(
      (name, i) => (map[name] = CATEGORY_PALETTE[i % CATEGORY_PALETTE.length])
    );
    return map;
  }, [overviewCategory]);

  // 테이블 페이지네이션 (이제 "거래 건" 기준)
  ...
  const fetchSales = async (from, to) => {
    ...
    try {
      const [listRes, weekRes] = await Promise.all([
        axios.get("/api/purchases/range", { params: { from, to } }),  // ✅ 수정 완료
  ...
  const orders = useMemo(() => {
    ...
    items.forEach((v) => {
      // 우선순위: session_code > purchase_session_id > 시간
      const sessionKey =
        v.session_code ||
        (v.purchase_session_id ? `SESSION-${v.purchase_session_id}` : null);

      const rawTime = v.purchased_at_kst || v.purchased_at || v.date || "";

      const key = sessionKey || rawTime; // 세션 없으면 예전처럼 시간 기준 fallback
      ...
      const g = map.get(key);
      g.items.push(v);
      g.totalAmount += Number(v.total_price || 0);
      g.totalQuantity += Number(v.quantity || 0);
      if (!g.payment_method && v.payment_method) {
        g.payment_method = v.payment_method;
      }
    });

    return Array.from(map.values()).sort((a, b) =>
      a.purchased_at_kst < b.purchased_at_kst ? 1 : -1
    );
  }, [items]);
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              <div className="relative bg-white p-8 rounded-2xl shadow-sm border border-gray-200">
                ...
                <div className="mt-2">
                  <span className="text-5xl font-extrabold tracking-tight text-gray-900">
                    {Number(
                      overviewSummary.total_quantity || 0
                    ).toLocaleString()}
                  </span>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-gray-800 text-base font-extrabold">
                    상품 분류별 판매현황
                  </p>
                  <span className="text-xs text-gray-400">
                    합계{" "}
                    {Number(
                      overviewCategory.reduce(
                    ...
                    <PieChart>
                      <Pie
                        ...
                        label={({
                          ...
                          const x =
                            cx + r * Math.cos(-midAngle * RAD);
                          const y =
                            cy + r * Math.sin(-midAngle * RAD);
                          return (
                            <text
                              x={x}
                              y={y}
                      ...
                      >
                        {overviewCategory.map((v, i) => (
                          <Cell
                            key={`${v.category}-${i}`}
                            fill={categoryColorMap[v.category]}
                          />
                        ))}
                      </Pie>
                    ...
                    domain={[
                      ...
                      Math.max(
                        ...
                        Math.ceil(
                          (Math.max(
                            0,
                            ...overviewWeekly.map((d) => +d.total || 0)
                          ) *
                            1.2) /
                            1000
                        ) * 1000,
            ...
            <section className="bg-white p-6 rounded-lg shadow">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                <p className="text-gray-700 font-semibold">
                  {/* 거래 건 기준 + 상품 개수도 같이 표시 */}
                  총 {orders.length}건 (상품 {items.length}개)
                </p>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm text-left text-gray-700 border-t">
                  <thead className="bg-gray-100 border-b sticky top-0">
                    <tr>
                      <th className="px-4 py-2">판매일시</th>
                      ...
                      <React.Fragment key={order.key}>
                        ...
                        <tr className="bg-slate-50 border-b">
                          <td className="px-4 py-2 font-semibold">
                            {formatDateTime(order.purchased_at_kst)}
                          </td>
                          <td className="px-4 py-2 text-gray-700" colSpan={2}>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mt-4">
                <p className="text-sm text-gray-600">
                  {orders.length === 0
                    ? "표시할 데이터가 없습니다."
                    : `${(start + 1).toLocaleString()}–${end.toLocaleString()} / ${orders.length.toLocaleString()} 건`}
                </p>
